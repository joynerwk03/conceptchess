"""Fit the proximity curve, monotonically, instead of forcing it quadratic.
...
def _apply(vec):
    ...
    n = _SPEC["n"]
    for u in range(n):
        ka.PX_MG[u] = float(vec[u])
        ka.PX_EG[u] = float(vec[n + u])
...
def main():
    ...
    ap.add_argument("--out", default=str(ROOT / "research/data/prox_fit.json"))
    a = ap.parse_args()

    from engine.concepts.king_attack import PX_MG, PX_EG, PX_MAX
    ...
    n = PX_MAX
    cur = np.concatenate([np.asarray(PX_MG, float), np.asarray(PX_EG, float)])
    print(f"{cut} fitting positions, {len(rows)-cut} held out")
    print(f"{2*n} parameters (danger by closeness units, middlegame and endgame)")
    ...
    json.dump({"PX_MG": [float(x) for x in cur[:n]],
               "PX_EG": [float(x) for x in cur[n:]]}, open(a.out, "w"), indent=1)
